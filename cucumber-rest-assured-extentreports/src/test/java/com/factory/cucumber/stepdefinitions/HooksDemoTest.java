...
public class HooksDemoTest {
	...
	public void setAPIEndpointURL() {
		...
		GetAPITest.apiEndPointUri = String.format("%s%s", apiHostName, "/api/users?page=2");
		logger.info("This step runns before All steps of a scenario");
		logger.info("Cucumber Hostname URL is :: " + GetAPITest.apiEndPointUri);
	...
	public void verifyStatusCode() {
		...
		if (statusCode.equals(String.valueOf(GetAPITest.STATUS_CODE))) {
			logger.info("This step runns after All steps of a scenario");
			Assert.assertEquals(GetAPITest.STATUS_CODE, statusCode);
			logger.info("Status Code is :: " + GetAPITest.STATUS_CODE);
		} else {
			Assert.assertEquals(GetAPITest.STATUS_CODE, statusCode);
			logger.info("Status Code is not as expected :: " + GetAPITest.STATUS_CODE);
		}
	}
